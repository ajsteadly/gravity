...
public class SimplePendulum extends AbstractPendulum {
     ...
     */
    public SimplePendulum (double inLength, double inMass, double inTheta0, GravityModel inModel) {
	super (inLength, inMass, inTheta0, inModel);
	angularFrequency = Math.sqrt (getGravityModel().getGravitationalField () / this.getStringLength ());
	periodOfMotion = 2 * Math.PI 
	    * Math.sqrt (this.getStringLength () / getGravityModel().getGravitationalField ());
