...
public class PendulumRunner {
	...
	GravityModel model = new GravityConstant(GRAVITY);
	RegularPendulum rp = new RegularPendulum (sLen, pMass, theta0, model, delta);
	SimplePendulum sp = new SimplePendulum (sLen, pMass, theta0, model);
	RegularPendulum rpCoarse = 
	    new RegularPendulum (sLen, pMass, theta0, model, .1);
