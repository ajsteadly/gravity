...
public class RegularPendulum extends AbstractPendulum {
     ...
     */
    public RegularPendulum (double inLength, double inMass, double inTheta0, GravityModel inModel,
		     double inDelta, double inDiss) {
	super (inLength, inMass, inTheta0, inModel);
	delta=inDelta;
	dissipation = inDiss;
	lastVel = 0;
	lastTheta = this.getMaxAngularDisplacement ();
	lastAccel = -(getGravityModel().getGravitationalField () / this.getStringLength ())*Math.sin (lastTheta);
    }

    public RegularPendulum (double inLength, double inMass, double inTheta0, GravityModel inModel,
		     double inDelta) {
	this (inLength, inMass, inTheta0, inModel, inDelta, 0);
    }
	...
	lastAccel = - dissipation*lastVel - getGravityModel().getGravitationalField () / this.getStringLength () * Math.sin (lastTheta);
    }
